...
def wrapper_overlap_region(func):
    '''Wrap a region generator to remove all regions that do not overlap a mask

    This function wraps the region generator ``func``. Regions are
    generated by that function, but are then additionally filtered
    such that regions that do not include at least one pixel where
    `self.region_to_use` is ``True`` are skipped.

    Parameters
    ----------
    func : callable
        function to be wrapped.
    region_to_use : np.array
        Mark an area as `True` in this array. Only regions overlapping with
        the array set to `True` will be returned.

    Returns
    -------
    func_check_overlap : function
        wrapped ``func``
    '''
    def func_check_overlap(self):
        for r in func(self):
            r = self.anyreg_to_mask(r)
            if (self.region_to_use.flatten() & r).sum() >= 1:
                yield r
    return func_check_overlap
